...
def generate(rng, count, lengths, depths, seen, name, pair_mode='exclude'):
    ...
    while len(rows)<count:
        ...
        answer=functional_oracle(program,data)
        # Long filter pipelines often collapse to []. Balance outcomes so this
        # shortcut is visible and cannot dominate the headline accuracy.
        want_empty=len(rows)%4==0
        if (len(answer)==0)!=want_empty:continue
        seen.add(key)
        rows.append(dict(id=f'{name}_{len(rows)}',program=program,data=data,
                         answer=answer,length=n,depth=k))
    return rows
...
def prepare():
    ...
    manifest=dict(seed=SEED,hashes=hashes,counts={k:len(v) for k,v in sets.items()},
                  ...
                  empty_answer_fraction_except_stress=0.25,
