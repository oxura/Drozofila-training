...
def variants(graph_mode='history'):
    return {
...
def configuration(name, settings, seed, steps, eval_every):
    return dict(variant=name, **settings, seed=seed, steps=steps, eval_every=eval_every,
                batch_size=16, lr=.0005, threads=2)
...
def freeze(steps):
    ...
    runs = {name + f'_seed{seed}': configuration(name, value, seed, steps, 2000)
            for name, value in variants(graph_mode).items() for seed in [0, 1, 2]}
    ...
    plan = dict(frozen_utc=datetime.now(timezone.utc).isoformat(), runs=runs, graph_mode=graph_mode,
                pilot_results=pilot_data, source_hashes={s: digest(ROOT / s) for s in SOURCES},
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(); p.add_argument('action', choices=['pilots', 'freeze', 'train', 'evaluate'])
