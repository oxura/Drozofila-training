...
def validate(model,rows,mode):
    model.eval();outputs=generate(model,[r['prompt'] for r in rows],max_new_tokens=72)
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--mode',choices=['direct','trace'],default='trace');p.add_argument('--condition',choices=['mlp','fly','rewired','no_edges'],default='mlp')
