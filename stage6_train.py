...
def train(config,out,resume=None):
    ...
    atomic_start=len(rows);atomic_manifest=None
    if config.get('atomic',False):
        atomic_manifest=json.loads((DATA/'atomic_manifest.json').read_text())
        assert hashlib.sha256((DATA/'atomic_train.jsonl').read_bytes()).hexdigest()==atomic_manifest['sha256']
        rows=rows+load('atomic_train')
    ...
    for family in families:
        ...
        for limit in range(1,max_difficulty+1):
            eligible=[i for i,r in enumerate(rows[:atomic_start]) if r['family']==family and r['difficulty']<=limit]
    ...
    atomic_pools={f:np.array([i for i in range(atomic_start,len(rows)) if rows[i]['family']==f]) for f in families}
    ...
    if resume:
        ...
        assert saved.get('atomic_data_hash')==(atomic_manifest['sha256'] if atomic_manifest else None)
    ...
    t0=time.perf_counter();losses=[];max_edge_grad=0;family_counts=defaultdict(int);atomic_examples=0
    for step in range(start,config['steps']+1):
        ...
        if config.get('atomic',False) and rng.random()<(.75 if step<=.4*config['steps'] else .25):
            chosen=rng.choice(atomic_pools[family],size=config['batch_size']);atomic_examples+=len(chosen)
        else:
        ...
        if step%config['eval_every']==0 or step==config['steps']:
            ...
            checkpoint=dict(config=config,model=model.state_dict(),optimizer=optimizer.state_dict(),step=step,best_score=list(best),
                            ...
                            atomic_data_hash=atomic_manifest['sha256'] if atomic_manifest else None)
    ...
    result=dict(config=config,best_step=checkpoint['step'],validation_score=checkpoint['best_score'],
                ...
                family_examples_this_invocation=dict(family_counts),atomic_examples_this_invocation=atomic_examples,
                atomic_data_hash=atomic_manifest['sha256'] if atomic_manifest else None,dataset_hashes=manifest['hashes'],final_test_read=False)
...
def main():
    ...
    name=f"{args.mode}_{'copy_' if args.copy else ''}{'atomic_' if args.atomic else ''}{args.condition}_seed{args.seed}"
