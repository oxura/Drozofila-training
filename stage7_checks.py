...
def check_models():
    ...
    grown = models['wider']; optimizer = torch.optim.AdamW(grown.parameters(), lr=.0005)
    grown_gradients = {}
    for _ in range(2):
        optimizer.zero_grad(); p, _ = grown(ids, ids != 0, pm)
        torch.nn.functional.nll_loss(p.log().transpose(1, 2), labels, ignore_index=-100).backward()
        for i, block in enumerate(grown.blocks):
            for key, value in [('incoming', block.ff[0].weight.grad[312:]), ('outgoing', block.ff[2].weight.grad[:, 312:])]:
                name = f'block{i}_{key}'
                grown_gradients[name] = max(grown_gradients.get(name, 0.), float(value.abs().max()))
        optimizer.step()
    assert all(x > 0 for x in grown_gradients.values())
    result['wider_new_unit_gradients'] = grown_gradients
    result['verification_optimizer_updates'] = 4
