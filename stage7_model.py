...
def initialize(config):
    ...
    if config.get('wider', False):
        # Independent incoming features, initially zero outgoing weights. Exact
        # duplication would stay symmetric under this deterministic optimizer,
        # so it would not be a valid control for extra effective capacity.
        for i in range(3):
            prefix = f'blocks.{i}.ff.'
            fresh = model.blocks[i].ff
            state[prefix + '0.weight'] = torch.cat([state[prefix + '0.weight'], fresh[0].weight.detach()[312:]], 0)
            state[prefix + '0.bias'] = torch.cat([state[prefix + '0.bias'], fresh[0].bias.detach()[312:]], 0)
            state[prefix + '2.weight'] = torch.cat([state[prefix + '2.weight'], torch.zeros(96, 8)], 1)
